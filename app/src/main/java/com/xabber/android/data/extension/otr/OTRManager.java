...
public class OTRManager implements OtrEngineHost, OtrEngineListener,
    ...
    private static Map<SecurityOtrMode, OtrPolicyImpl> POLICIES;

    static {
        POLICIES = new HashMap<>();
        POLICIES.put(SecurityOtrMode.disabled, new OtrPolicyImpl(OtrPolicyImpl.NEVER));
        POLICIES.put(SecurityOtrMode.manual, new OtrPolicyImpl(OtrPolicyImpl.OTRL_POLICY_MANUAL & ~OtrPolicyImpl.ALLOW_V1));
        POLICIES.put(SecurityOtrMode.auto, new OtrPolicyImpl(OtrPolicyImpl.OPPORTUNISTIC & ~OtrPolicyImpl.ALLOW_V1));
        POLICIES.put(SecurityOtrMode.required, new OtrPolicyImpl(OtrPolicyImpl.OTRL_POLICY_ALWAYS & ~OtrPolicyImpl.ALLOW_V1));
    }
    ...
    @Override
    public OtrPolicyImpl getSessionPolicy(SessionID sessionID) {
    ...
    public String transformSending(String account, String user, String content) throws OtrException {
        ...
        String parts[] = getOrCreateSession(account, user).transformSending(content, null).split(" ");
